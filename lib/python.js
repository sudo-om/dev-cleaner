...
export async function cleanPython() {
  const output = execSync(
    'find ~/Developer -type d \\( -name "venv" -o -name ".venv" \\) -not -path "*/site-packages/*" -exec du -sh {} + | sort -hr | head -n 20',
  ...
  const ok = await confirm({
    message: `Delete ${selected.length} environments?`,
    default: false
  });
